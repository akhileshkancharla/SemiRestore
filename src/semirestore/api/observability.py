...
async def observe_inference(
    ...
    try:
        ...
        emit_event(
            ...
            "inference_cancelled",
            **common,
            inference_duration_ms=_elapsed_ms(start_ns),
            outcome="cancelled",
            model_readiness="ready",
        )
        raise
    except InferenceBusyError:
        _emit_inference_outcome(common, start_ns, "busy", ErrorCode.INFERENCE_BUSY, "ready")
        raise
    except InferenceTimeoutError:
        _emit_inference_outcome(
            common, start_ns, "timeout", ErrorCode.INFERENCE_TIMEOUT, "ready"
        )
        raise
        ...
        _emit_inference_outcome(
            common, start_ns, "failed", ErrorCode.RESTORATION_FAILED, "ready"
        )
        raise
    except Exception:
        _emit_inference_outcome(common, start_ns, "failed", ErrorCode.INTERNAL_ERROR, "ready")
        raise

    _emit_inference_outcome(common, start_ns, "success", None, "ready")
    return result
